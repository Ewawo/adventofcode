...
public class Y24D22P2 extends Day {
    ...
    protected Object logic() {
        ...
        for (String line : input) {
            ...
            calculateSecret(Long.parseLong(line), 2000, temp);
            mergeMaps(map, temp);
        }
    ...
    private void mergeMaps(Map<Integer, Long> map1, Map<Integer, Long> map2) {
        for (int key : map2.keySet()) {
            map1.merge(key, map2.get(key), Long::sum);
        }
    }

    private void calculateSecret(long secret, int times, Map<Integer, Long> map) {
        ...
        for (int i = 0; i < times; i++) {
            ...
            int key = formatKey(diff[Math.abs(p - 3) % 4], diff[Math.abs(p - 2) % 4], diff[Math.abs(p - 1) % 4], diff[p % 4]);
    ...
    private int formatKey(long v1, long v2, long v3, long v4) {
